...
public class SearchOptions
{
	...
	public String SearchUCOps2(String category, String city, String model, int sortC) 
	{
		String ret = "";
		String sql = pickSortingOp2(sortC);
		String[] CCM = new String[3];
		if(category.replaceAll("\\s+","").isEmpty() && city.replaceAll("\\s+","").isEmpty() && model.replaceAll("\\s+","").isEmpty())
		{
			System.out.println("Searching for all UUber Cars");
			ret = executeUCSearchQ(CCM, sql, 0);
		}
		//Only by category
		else if(!category.isEmpty() && city.isEmpty() && model.isEmpty())
		{
			...
			sql += " AND category = ?";
			CCM[0] = category;
			ret = executeUCSearchQ(CCM, sql, 1);
		}
		//Only by city
		else if(!city.isEmpty() && category.isEmpty() && model.isEmpty())
		{
		...
		else if(!category.isEmpty() && !city.isEmpty() && model.isEmpty())
		{
			...
			CCM[0] = category;
			CCM[1] = city;
			ret = executeUCSearchQ(CCM, sql, 2);
		}
		//Category and/or model
		else if(!category.isEmpty() && !model.isEmpty() && city.isEmpty())
		{
		...
		else
		{
			...
			CCM[1] = city;
			CCM[2] = model;
			ret = executeUCSearchQ(CCM, sql, 3);
	 ...
	 */
	private String executeUCSearchQ(String[] CCM, String sql, int size)
	{
		try 
		{	
			...
			try(PreparedStatement pstmt = con.con.prepareStatement(sql))
			{
				...
				return printUCSearchResult(result);
			} 
	 ...
	 */
	private String printUCSearchResult(ResultSet result)
	{
		String ret = "";
		try
		{
			if(result.isBeforeFirst())
			{
				...
				while(result.next())
				{
					...
					+ "<b>    Average Score: </b>" + result.getString("avScore") + "<BR><BR>";
			...
			{
				ret += "There are no cars with your specifications";
	 ...
	 */
	private String pickSortingOp2(int c)
	{
		String sql = null;
       	if (c==1 | c==2)
       	{
	 ...
	 */
	public String viewUsefulFeedbacks(String login, int m)
	{
		String ret = "";
       	try 
		{	
			...
			try(PreparedStatement pstmt = con.con.prepareStatement(sql))
			{
				...
				if(result.isBeforeFirst())
				{
					...
					while(result.next())
					{
						ret += "<b>#" + Integer.toString(rank) + " fid: </b>" + result.getString("fid")
						+ "<b><BR>Usefulness Score: </b>" + result.getString("avUsefulness")
						+ "<b> User giving feedback: </b>" + result.getString("login")
						+ "<b> Score given: </b>" + result.getString("Score")
						+ "<b> Comment: </b>" + result.getString("text")
						+ "<b> Vehicle #: </b>" + result.getInt("vin") + "<BR><BR>";
				...
				{
					ret += "There are no useful feedbacks for this user";
